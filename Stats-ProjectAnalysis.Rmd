...
\beta_{9} D_{\text{Color.YellowOrange i}} + \epsilon_i, \epsilon_i \sim_{\text{ind}} N(0, \sigma^2) \\
$$
Fitted
$$
...
```
```{r}
lm.obj <- lm(sqrt(Sweetness) ~ Weight + Color + Acidity + Softness + 
    HarvestTime + Weight:Color + Weight:Acidity + Color:Acidity + 
    Color:HarvestTime + Softness:HarvestTime, data = df2)
...
```{r}
advertising_minus_6 <- df2[-6, ]

lm.obj_reduced <- lm(Sweetness ~ Weight + Color + Acidity + Softness + 
    HarvestTime + Weight:Color + Weight:Acidity + Color:Acidity + 
    Color:HarvestTime + Softness:HarvestTime, data = advertising_minus_6)
...
log.obj <- glm(Blemishes ~ ., data = df1)
...
# Fit multinomial logistic regression model
log.obj <- glm(Blemishes ~ Size + Acidity + Softness + Ripeness + Weight, data = df1)
