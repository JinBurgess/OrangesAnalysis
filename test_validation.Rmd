...
```
```{r}
tree <- getTree(rfc_model, 1, labelVar=TRUE)
...
tree_structure <- getTree(tree_df, 1, labelVar = TRUE)

# Convert the tree structure to an rpart object
rpart_tree <- rpart::rpart(as.formula(tree_structure$frame$yval2 ~ .), 
                           data = as.data.frame(tree_structure$frame), 
...
tree <- getTree(rf_model, 1)

# Convert the tree to an rpart object
rpart_tree <- rpart::rpart(formula = as.formula(tree$frame$yval2 ~ .),
                           data = as.data.frame(tree$frame), 
                           control = rpart::rpart.control(minsplit = 1))
